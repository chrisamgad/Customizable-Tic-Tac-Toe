...
public class TicTacGame {
            ...
            if(AI_Find_Opportunity()==true)
            {
            ...
            else //AI_Find_Opportunity())==true
                {
                    ...
                    if (BlockOponent==true) 
                        ...
                        {
                            AI_Play_Move();// AI plays the move
                            //System.out.println("AI WILL PLAY THE MOVE AS NO BLOCKOPENT NOR WINNING OPPORTUNITY WAS FOUND");//Uncomment this System.out Line  to debug the AI MOVE Feature
                        }
                }
        }
        CheckIfWinorDraw();
    ...
    public boolean AI_Find_Opportunity() // finds a winning oportunity
    {
        ...
        //  Try finding a winning Opportunity vertically and if detected place it 
                ...
                for(int j=0;j<k;j++)
                    {
                        ...
                        {   
                            ...
                            flag=false;
                            for(int z=0;z<k;z++) // a for loop that breaks that goes through the cells with sybmols and if detects an X, breaks to give chance to detect other cases,to find a verical winning scenario
                            {   
        ...
        while(CellsStates[CalculatedIndex]!=CellState.Empty && CurrentBoardState!=BoardState.DRAW )
        {
		    ...
		    {   
                    ...
                    while(CellsStates[CalculatedIndex]!=CellState.Empty && CurrentBoardState!=BoardState.DRAW ) // while there are no empty cells and game still didnt draw
                        {
                            //CalculatedIndex=rand.nRextInt(n*m);// Assign Calculated Index to a random number
      ...
      if(winOppForenemy==false) // if still no block opportunity detected
        {
            ...
            {   index=i;
                ...
                for(int j=0;j<k;j++)
                    ...
                    if(X_Counter==k-1)
                        {   //If X_Counter reaches number of symbols to win 
                                //Create an X variable that stores IndexTemp+X_Counter
            ...
            if(winOppForenemy==true) // if true, we go over the row to see which cell was the empty one and fill it to block the enemy from playing in it and winning
            {    
